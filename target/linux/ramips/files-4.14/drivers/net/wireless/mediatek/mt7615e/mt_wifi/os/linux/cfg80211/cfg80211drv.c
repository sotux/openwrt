		...
		BOOLEAN                                          isReg)
{
		...
		(pDevEntry != NULL))
	{
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("[%d] TYPE pDevEntry->Cfg80211RegisterActionFrame=%d[%d]\n",
		...
		for(IdChan=0; IdChan<ChanNum; IdChan++)
		{
			...
			for(IdPwr=0; IdPwr<MAX_NUM_OF_CHANNELS; IdPwr++)
			{
				...
				//if (ChanId == pAd->TxPower[IdPwr].Channel)
				{
					...
					/*os_move_mem(&pAd->ChannelList[RecId],
									&pAd->TxPower[IdPwr],
