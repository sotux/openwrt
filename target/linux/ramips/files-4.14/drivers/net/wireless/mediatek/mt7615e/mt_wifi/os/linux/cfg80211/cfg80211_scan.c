	...
	IN VOID							*pAdCB)
{
	...
	for (index = 0; index < pAd->ScanTab.BssNr; index++)
	{
		...
		if (pAd->ScanTab.BssEntry[index].Channel > 14)
			CenFreq = ieee80211_channel_to_frequency(pAd->ScanTab.BssEntry[index].Channel , MT76_BAND_5GHZ);
		else
			CenFreq = ieee80211_channel_to_frequency(pAd->ScanTab.BssEntry[index].Channel , MT76_BAND_2GHZ);
#else
